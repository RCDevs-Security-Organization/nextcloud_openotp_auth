...
class OpenotpAuth
{
	...
	/** NuSOAP object */
	private	nusoap_client	$soap_client;
	...
	// public function getServer_urls(): array
	// {
	 ...
	 * @return array{domain: string|null, username: string}|string 
	 */
	...
	public function getContext_name(): string
	{
		return $this->context_name;
	}
	public function getContext_size(): int
	{
		return $this->context_size;
	}
	public function getContext_time(): int
	{
	...
	private function soapRequest(int $serverId): bool
	{
		...
		$soap_client = new nusoap_client(
			$this->server_urls[$serverId],
			false,
			$proxyHost ?: '',
			$proxyPort ?: '',
			$proxyUsername ?: '',
	 ...
	 */
	public function openOTPSimpleLogin(
		string		$username,
		string		$domain,
		...
		for ($i = 1; $i <= self::NB_SERVERS; $i++) {
			...
			$resp = $this->soap_client->call('openotpSimpleLogin', array(
				...
				'apiKey' => $this->api_key,
				'source' => $_SERVER['REMOTE_ADDR'] ?? '0.0.0.0',
			...
			if ($this->soap_client->fault) {
				// Safely extract fault code/string from mixed $resp
				$faultCode = (is_array($resp) && isset($resp['faultcode']) && is_string($resp['faultcode'])) ? $resp['faultcode'] : 'unknown';
				$faultStr  = (is_array($resp) && isset($resp['faultstring']) && is_string($resp['faultstring'])) ? $resp['faultstring'] : 'unknown';

				$this->logger->error(
					sprintf('%s, error: %s / %s', __METHOD__, $faultCode, $faultStr),
					['app' => OpenOTPAuthApp::APP_ID]
				);
				return false;
			}
			...
			if (is_string($err) && $err !== '') {
				$this->logger->error(
					sprintf('%s, error: %s', __METHOD__, $err),
					['app' => OpenOTPAuthApp::APP_ID]
				);
				continue;
			}
	 ...
	 * @return array<string, mixed>|false 
	 */
	public function openOTPChallenge(
		string		$username,
		string		$domain,
		string		$state,
		string|null	$password,
